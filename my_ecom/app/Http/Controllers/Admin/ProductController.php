    ...
    public function manage_product_process(Request $request)
    {
        ...
        if ($request->hasfile('image')) {
            if ($request->post('id') > 0) {
                ...
                if (Storage::exists('/public/upload/product/' . $arrImage[0]->image)) {
                    Storage::delete('/public/upload/product/' . $arrImage[0]->image);
                }
            }
            $image = $request->file('image');
            $ext = $image->extension();
            $image_name = time() . '.' . $ext;
            $image->storeAs('/public/upload/product/', $image_name);
        ...
        foreach ($skuArr as $key => $val) {
            ...
            if ($request->hasFile("attr_image.$key")) {
                if ($paidArr[$key] != '') {
                    ...
                    if (Storage::exists('/public/upload/product/' . $arrImage[0]->attr_image)) {
                        Storage::delete('/public/upload/product/' . $arrImage[0]->attr_image);
                    }
                }
                ...
                $image_name = $rand . '.' . $ext;
                $request->file("attr_image.$key")->storeAs('/public/upload/product/', $image_name);
        ...
        foreach ($piidArr as $key => $val) {
            ...
            if ($request->hasFile("images.$key")) {
                ...
                if ($piidArr[$key] != '') {
                    ...
                    if (Storage::exists('/public/upload/product/' . $arrImage[0]->images)) {
                        Storage::delete('/public/upload/product/' . $arrImage[0]->images);
                    }
                }
                if (!Storage::disk('public')->exists('upload/product')) {
                    Storage::disk('public')->makeDirectory('upload/product');
                }
                $rand = rand('111111111', '999999999');
                $images = $request->file("images.$key");
                $ext = $images->extension();
                $image_name = $rand . '.' . $ext;
                $request->file("images.$key")->storeAs('/public/upload/product/', $image_name);
