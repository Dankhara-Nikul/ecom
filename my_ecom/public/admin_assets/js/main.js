...
function order_status() {
    var x = document.getElementById('bluk');
    var order_status_id = x.value;
    var order_status = jQuery('#bluk').val();
    ...
    if (order_status > 0) {
        if (order_id.length > 0) {
            var check = confirm('Are your sure change order status? and order id is ' + order_id.join(", "));
            order_id.forEach(myFun);
        } else {
    ...
    function myFun(value, index, array) {
        if (check == true) {
            ...
            window.location.href = '/admin/update_order_status1/' + order_status + '/' + value;
        }
    }
}

function order_filter() {
    var order_status_id = document.getElementById('bluk').value;
    //  var order_status_id = x.value;
    //  var order_status = jQuery('#bluk').val();
    var date = document.getElementById('date').value;
    // var date1 = date.value;

    if (order_status_id == 0 || date == 0) {
        alert("must be Select the order status and date both.After you can Filter");
    } else {
        window.location.href = '/admin/update_order_filter/' + order_status_id + '/' + date;
    }
}

function order_clear_filter() {
